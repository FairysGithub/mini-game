...
var update = function (modifier) {
    ...
    if (dave.y < canvas.height-32 && 40 in keysDown) { // Player holding down
    dave.y += dave.speed * modifier;
    }
    else if (dave.y >=canvas.height-32 && 40 in keysDown) { // Player holding down
    timeLeft=0;
    daveImage.src = "images/dead.png";
    }
    ...
    if(dave.x< canvas.width-32 && 39 in keysDown) { // Player holding right
    dave.x += dave.speed * modifier;
    }
    else if(dave.x>=canvas.width-32 && 39 in keysDown){
    timeLeft=0;
    daveImage.src = "images/dead.png";
    }
    //Are they touching?
    if(dave.x<=(monster.x+32)&&monster.x<=(dave.x+32)&&dave.y<=(monster.y+32)&&monster.y<=(dave.y+32)){
        ++monstersCaught;
        reset();
    }
}
...
reset();
var then = Date.now();
var timerGameMain=setInterval(main, 1000/60); // Execute as fast as possible
